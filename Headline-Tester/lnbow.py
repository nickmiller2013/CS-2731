...
if __name__ == '__main__':
    ...
    for train_ex in train_data:
        ...
        tag_head = nltk.pos_tag(word_tokenize(headline))
        
        count = 0
        for (word, tag) in tag_head:
            if tag not in ('CC','IN','TO','DT','WDT','WP','WP$','PRP','PRP$'):
                bow.add(word)
        ...
        if train_ex['Stance'] == 'unrelated':
            unrel_av = unrel_av + count
            unrel_count = unrel_count + 1
        else:
            rel_av = rel_av + count
            rel_count = rel_count + 1
        if (rel_count + unrel_count)%1000 == 0:
            print rel_count + unrel_count
